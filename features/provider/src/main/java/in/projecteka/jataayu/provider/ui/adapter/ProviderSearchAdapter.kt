package `in`.projecteka.jataayu.provider.ui.adapter
...
import `in`.projecteka.jataayu.util.extension.findView
...
class ProviderSearchAdapter(itemClickCallback : ItemClickCallback, list : List<ProviderInfo>) :
    ...
    override fun onBindViewHolder(holder : RecyclerViewHolder, position : Int) {
        ...
        searchResultView.text = highlight(query.toString(),
            if (providerInfo.hip.name.isEmpty()) searchResultView.getString(R.string.no_results_found)
    ...
    private fun getDisplayName(originalText : String, search : String, startingIndex : Int,
                               context : Context) : String {
        if (startingIndex == -1) return originalText
        return originalText.replaceFirst(
            search, formatBold(
                originalText.substring(startingIndex, (startingIndex + search.length)), context),
            true)
    }
